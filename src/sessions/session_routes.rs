    ...
    req_session
        ...
        .and_then(move |auth_opt| {
            ...
            if let Some(auth) = auth_opt {
                Either::A(
                    is_valid(&auth, &session_mgr).and_then(move |sign_in_valid: bool| {
                        info!(
                        ...
                        if sign_in_valid {
                            ...
                            req_session.flash("You've been signed out by another location.")?;
...
fn login_google(request: &HttpRequest<State>) -> Result<HttpResponse> {
    let redirect_uri = format!(
        "https://{}/login/google/callback",
        request.state().config.http_host()
    );
    ...
    Ok(HttpResponse::Found()
        .header(
            ...
            get_redirect_url(
                &redirect_uri,
                request.state().config.google_oauth_client_id(),
                None,
                Some("reaktor.fi"),
            ),
        )
...
fn login_google_callback(
    ...
    let host = request.state().config.http_host();
    let token = google_oauth::exchange_code_for_token(request.state(), &code).and_then(move |result| {
